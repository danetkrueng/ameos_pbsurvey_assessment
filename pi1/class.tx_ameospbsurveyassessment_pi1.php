...
class tx_ameospbsurveyassessment_pi1 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		$ses					= $GLOBALS['TSFE']->fe_user->getKey('ses','surveyData');
		$this->iResId = $ses["rid"];
	...
	function _groupScoring() {
		$mod					= $this->conf['groupcount'];
		...
		if ( strtolower( $this->conf['resultordering'] ) == 'desc' ) {
			array_multisort($aWeight, SORT_DESC, $aFinalAnswers);
		} elseif ( strtolower( $this->conf['resultordering'] ) == 'asc' ) {
